...
=item B<--minimalContigDepth> INT
 ...
 Contig's minimum depth (-i) [8]

=item B<-c, --maxDepth> INT

 Coverage, maximum depth use to filter input reads (-i) [300]
...
sub check {
	...
	# Step1 Assemble
	print "\n- Step 1 - assemble:\n";
	my $err = `which cutadapt > /dev/null 2>&1`;
	print "  => cutadapt.........";
	if($?) { print colored(['bold red'], 'unavailable in the PATH!', "\n"); }
	else   {
		print colored(['bold green'], "ok");
		my $version = `cutadapt --version`;
		chomp($version);
		print "...version:$version\n";
	}

	$err = `which spades.py > /dev/null 2>&1`;
	print "  => spades...........";
	if($?) { print colored(['bold red'], 'unavailable in the PATH!', "\n"); }
	else   {
		print colored(['bold green'], "ok");
...
sub assemble {
	...
	if($#a_assemblySeq != -1) {
		...
		foreach my $k (sort keys(%h_sample)) {
			...
			open(FASTA, $h_sample{$k}{"assembly"}) || die "Error: Unabled to open $h_sample{$k}{"assembly"}\n";
    		...
    		my $name=$1;
				$entry=~s/\n//g;
				print OUTFASTA $name."#".length($entry)."\n$entry\n";
  		}
			$/="\n";
			close FASTA;
			close OUTFASTA;
